...
const resolvePath = (path, filePath) => {
    ...
    const deepSize = filePath.replace(new RegExp(regex), '$2').split('/').length - 2; // 2 = 1 (split extra) + 1 (path extra)
    const symbol = '../'

    return path.replace(pathToResolve, `${symbol.repeat(deepSize)}${mappings[pathToResolve]}`);
};
